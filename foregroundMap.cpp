...
//add to first row of the map
	for (int i = 0; i < 20; i++) {
		...
		this->map[i][0][0] = unit;
		std::cout << "Foreground Map tile" << i << " 0 = " << unit << std::endl;
...
//add to last row of the map
	for (int i = 0; i < 20; i++) {
		...
		std::cout << "Foreground Map tile" << i << "  1 = " << unit
				<< std::endl;
		...
		//if index has another unit of the other player's and a unit was clicked previously, attack the other unit
			...
			if ((player == 1 && this->player2->containsUnit(clicked))
				...
				int distance = abs(this->clickedX - xIndex)
						+ abs(this->clickedY - yIndex);
				std::cout << "Range begins at: "
						<< this->unitClicked->getRangeBegins()
						<< "\nRange ends at: "
						<< this->unitClicked->getRangeEnds()
						<< "\nDistance to attacked unit: " << distance
						<< std::endl;
				if ((this->unitClicked->getRangeBegins() <= distance)
					...
					std::cout << "A unit has been attacked!" << std::endl;
