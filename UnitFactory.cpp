...
Unit * UnitFactory::createUnit(int xPos, int yPos, int height, int width, int player, int faction, int unitType, ImageHandler * imgHandler, SpecialAbilities* sa) {
    ...
    if (faction == 1) { // the british
        ...
        if (unitType == 0) { // men at arm
            ...
            return new Unit(xPos, yPos, height, width, "Characters/" + colour + "Knight.png", imgHandler, 55, 20, 5, 2, 1, 1, 4, new SpecialAbilities(100, 15, 0, 0, -1, 0, 2, 4), "Hussar");
        }
    }else if (faction == 3) { // the germans
        if (unitType == 0) { // teutonic knight
            return new Unit(xPos, yPos, height, width,"Characters/" + colour + "General.png", imgHandler, 120, 20, 5, 2, 1, 1, 2, new SpecialAbilities(30, 10, 0, 0, 0, 0, 2, 2), "Teutonic Knight");
            ...
            return new Unit(xPos, yPos, height, width,"Characters/" + colour + "Knight.png", imgHandler, 70, 10, 3, 4, 2, 3, 3, new SpecialAbilities(-3, 0, 1, 1, 1, 1, 3, 2), "Turcopole");
    }
    } else if (faction == 4) { // the spanish
        if (unitType == 0) { // conquistador
            return new Unit(xPos, yPos, height, width,"Characters/" + colour + "General.png", imgHandler, 65, 12, 6, 3, 2, 2, 4, new SpecialAbilities(-2, 0, 0, 0, 0, 0, 3, 1), "Conquistador");
        } else if (unitType == 1) { // arquebusier
            return new Unit(xPos, yPos, height, width,"Characters/" + colour + "Bandit.png", imgHandler, 30, 23, 1, 2, 2, 3, 4, new SpecialAbilities(100, 0, 0, 0, 0, 0, 0, 6), "Arquebusier");
