			...
			if (unitType == 0) { // men at arm
				...
				return new Unit(xPos, yPos, height, width,
                        ...
                        if (unitType == 0) { // conquistador
                                return new Unit(xPos, yPos, height, width,
                                                "Characters/" + colour + "Infantry.png", imgHandler, 65, 12, 6, 3,
                                                2, 2, 4);
                        } else if (unitType == 1) { // arquebusier
                                return new Unit(xPos, yPos, height, width,
                                                "Characters/" + colour + "Wrapman.png", imgHandler, 30, 23, 1, 2,
                                                2, 3, 4);
                        } else if (unitType == 2) { // jinete
                                return new Unit(xPos, yPos, height, width,
                                                "Characters/" + colour + "LightKnight.png", imgHandler, 60, 30, 3,
                                                2, 1, 1, 6);
                        }
                }
